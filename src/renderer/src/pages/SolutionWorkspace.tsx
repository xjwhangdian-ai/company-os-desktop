...
function buildSolutionPrompt(
  ...
  return [
    ...
    ``,
    `资料库（按需检索取材，不要整篇照搬）：`,
    `- 基础产品库：libraries/02_解决方案_solution/基础产品库/（共 ${libs.productLib} 份，用 Glob 列出后按相关性选读）`,
    `- 基础解决方案库：libraries/02_解决方案_solution/基础解决方案库/（共 ${libs.solutionLib} 份，找同类场景方案参考结构与打法）`,
    `- 政策文件库：libraries/02_解决方案_solution/政策文件库/（共 ${libs.policyLib} 份，方案涉及政策背景/合规依据时引用；情报线索卡只有链接没有原文，引用观点须注明"据线索，待核原文"）`,
    `- 行业趋势库：libraries/02_解决方案_solution/行业趋势库/（共 ${libs.trendLib} 份，写行业背景/市场趋势章节时取材，同样注意线索卡与原文的区别）`,
